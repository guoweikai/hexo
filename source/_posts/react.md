...
### 3. react 组件(组件是什么? 为什么拆分组件? 组件的功能有哪些, 难点有哪些和模块的区别)
    ...
    4. 组件实例的三大属性
       - state  (组件的状态是干什么的?如何往 state 放东西 如何修改状态?)
         this.setState()  调用时是合并的过程,并且是异步的, 可以不适用构造函数, 以及使用构造函数的问题
       -  props
          props 传递参数
          props 传递参数简写 {...props}
          props 的进行类型限制
       - 事件, 如何添加事件
         ...
         事件处理函数中处理函数.
       - refs:
         refs 的字符串定义, 目前不建议使用,后期会在某个版本中删除, 因为有内存消耗的问题.
         refs 的函数回调中进行使用, refs 的回调函数在更新时被调用两次.  直接写成内联的,和类绑定的回调.
         refs 的 createRef 的使用: React.createRef 调用后可以返回一个容器,该容器可以存储被 ref 表示的节点, 该容器是专人专用
...
### 7.React组件如何绑定事件, 和 vue 以及原生事件的不同?
...
### 8.在 React 中使用 原生事件不能冒泡 React 事件的原因
...
### 10. react 中的 React.createElement 都做了什么



### 11. ReactDom.render 做了什么呢?
...
### 12. React 的渲染流程可以分为两个阶段: 初始化阶段和更新阶段
1. 初始化阶段:
...
### 13. React  中 嵌套的组件虚拟 dom 是如何关联的呢
